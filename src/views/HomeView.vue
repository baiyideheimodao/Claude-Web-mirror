        ...
        <div class="mb-6">
          <div class="relative bg-white border border-[#e0e0df] rounded-xl shadow-sm hover:border-[#cfcfce] transition-colors duration-200">
            ...
            <div class="absolute bottom-3 left-3 right-3 flex items-center justify-between">
              ...
              <label
                v-if="!userInput"
                class="p-1.5 hover:bg-black/[0.04] rounded-md transition-colors duration-150 group cursor-pointer"
                title="Attach file"
              >
              ...
              <div class="flex items-center gap-0.5">
                <template v-if="!userInput">
                  <div
                    class="flex items-center gap-1 px-2 py-1.5 hover:bg-black/[0.04] rounded-md cursor-pointer transition-colors duration-150 group relative"
                    @click="showModelMenu = !showModelMenu"
                  >
                    <span class="text-[13px] text-[#787774] group-hover:text-[#5c5b58] leading-none tracking-tight">
                      {{ appStore.currentModel?.name || 'Sonnet' }}
                    </span>
                    ...
                    <div
                      v-if="showModelMenu"
                      class="absolute top-full right-0 mt-1 w-56 bg-white border border-[#e0e0df] rounded-lg shadow-claude-md py-1 z-50"
                    >
                      <button
                        v-for="model in appStore.models"
                        :key="model.id"
                        class="w-full px-3 py-2 text-left text-[13px] flex items-center justify-between hover:bg-black/[0.04] transition-colors"
                        :class="model.id === appStore.currentModel?.id ? 'text-[#d97757]' : 'text-[#1a1a1a]'"
...
const shortcuts = [
  { id: 1, label: 'Strategize', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>' },
  { id: 2, label: 'Write', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>' },
  { id: 3, label: 'Learn', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>' },
  { id: 4, label: 'Code', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>' },
  { id: 5, label: 'Life stuff', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M21 15.546c-.523 0-1.046.151-1.5.454a2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.701 2.701 0 00-1.5-.454M9 6v2m3-2v2m3-2v2M9 3h.01M12 3h.01M15 3h.01M21 21v-7a2 2 0 00-2-2H5a2 2 0 00-2 2v7h18zm-3-9v-2a2 2 0 00-2-2H8a2 2 0 00-2 2v2h12z"/></svg>' }
]
